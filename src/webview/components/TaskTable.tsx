...
const TaskTable: React.FC = memo(() => {
  ...
  const draggedTaskData = draggedTask ? tasks.find((t) => t.id === draggedTask) : undefined;
  ...
  const renderDropPreviewRow = (key: string, targetIndex: number, status?: StatusType) => (
    <tr
      key={key}
      onDragOver={(e) => handleDragOver(e as unknown as React.DragEvent, targetIndex)}
      onDrop={(e) => handleDrop(e as unknown as React.DragEvent, targetIndex)}
      className="leading-[10px]"
    >
      <td colSpan={4}>
        <div className="drop-preview-card">
          <div className="flex items-center gap-2 text-sm mb-1">
            <span className="inline-block w-2.5 h-2.5 rounded-full" style={{ backgroundColor: status?.color ?? "var(--vscode-statusBar-foreground)" }}></span>
            <span className="text-[var(--vscode-editor-foreground)]">{draggedTaskData?.text ?? "Drop here"}</span>
          </div>
          <div className="text-[var(--vscode-editorLineNumber-foreground)] text-xs">Drop position preview</div>
        </div>
      </td>
    </tr>
  );
        ...
        <tbody>
          ...
          {tasks.length > 0 ? (
            tasks.map((task, index) => {
              ...
              const showPreviewHere = dragOverIndex === index && draggedTask !== task.id;
              const statusForPreview = draggedTaskData ? findStatus(draggedTaskData.statusId) : findStatus(task.statusId);

              return (
                <Fragment key={task.id}>
                  {showPreviewHere && renderDropPreviewRow(`${task.id}-preview`, index, statusForPreview)}
          ...
          {draggedTask && dragOverIndex === tasks.length && renderDropPreviewRow("end-preview", tasks.length, draggedTaskData ? findStatus(draggedTaskData.statusId) : undefined)}
          ...
          {draggedTask && (
            ...
            >
              <td colSpan={4}>
                <div className={`drop-preview-card ${dragOverIndex === tasks.length ? "drop-preview-active" : ""}`}>
                  <div className="flex items-center gap-2 text-sm">
                    <span className="inline-block w-2.5 h-2.5 rounded-full" style={{ backgroundColor: draggedTaskData ? findStatus(draggedTaskData.statusId)?.color : "var(--vscode-statusBar-foreground)" }}></span>
                    <span className="text-[var(--vscode-editor-foreground)]">Drop at end</span>
                  </div>
                </div>
              </td>
            </tr>
